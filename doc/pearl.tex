...
zeroa and the array's size respectively.
%
...
as we have not yet found our candidate, and the subarray in focus is valid
...
assumption that the xinput array is sorted.

During such a proof we need to ensure that

the algorithm terminates by proving that the size of the subarray in focus
($\mathit{end} - \mathit{begin}$) is a strictly decreasing natural number

all of our array reads are valid i.e. that \textit{middle} is always a valid
index (in particular its computation never overflows)

the end value of \textit{found} is the correct answer to the decision problem
(this relies on the assumption that the array is sorted).
...
an adversarial manner, assuming they may break it at any time
%
