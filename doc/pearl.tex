...
available choice of effects in a given computation.
...
in the union.
%
This GADT has exactly one constructor that stores an \texttt{Int} --meant to be
understood as the index of the type of interest in $ts$-- together with a value
...
A simplified\footnote{
  ...
  and we have stripped annotations concerned with strictness or memory layout.
  }
...
The \texttt{Union} constructor is not exported so that users may only
manipulate elements of the open union type via the provided functions.
%
A library-wide invariant relying on strong assumptions about the host language
(namely global uniqueness of typeclass instances) guarantees that users of the
library will only ever be able to manufacture elements of the union where the
type of the value stored is equal to that pointed at by the \texttt{Int} index.
%
This justifies their widespread use of unsafe coercions when unpacking values of
the union type.

In Idris2 however we can take advantage of dependent types to implement a safe
version of \texttt{Union} where the invariant between the stored \texttt{Int},
the type of the value, and the list of types given as a parameter is explicit.
Additionally, we can take advantage of quantitative type theory to ensure that
this invariant will be runtime irrelevant thus obtaining a safe, proven correct
\emph{and} just as efficient version of the library.


% Their library provides this open union type as an Abstract Data Type
